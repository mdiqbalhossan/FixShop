$(function () {
    ...
    let customer_store = $("#customer_store").val();
    let price = $("#total_price").val();
    let payableAmount = $("#receivable_amount").val();
    let tax = $("#tax_amount").val();
    let discount = $("#discount").val();
    let receivedAmount = $("#received_amount").val();
    $("#product").autocomplete({
        ...
        select: function (event, ui) {
            if (ui.item.product_type == "variation") {
                ...
                html += `<td><span class="form-control fw-bold ${
                    ui.item.quantity_in_wirehouse > 0
                        ? "text-success"
                        : "text-danger"
                }">${ui.item.quantity_in_wirehouse} ${
                    ui.item.unit.name
                }</span></td>`;
